...
export const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case "REMOVE_TOAST": return toastId === undefined ? { ...state, toasts: [] } : { ...state, toasts: state.toasts.filter(t => t.id !== action.toastId) }
  }
}
...
function useToast() {
  ...
  React.useEffect(() => { listeners.push(setState); return () => { const idx = listeners.indexOf(setState); if (idx > -1) listeners.splice(idx, 1) } }, [state])
